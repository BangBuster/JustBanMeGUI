...
low(adapter).then(db => {
    app.post('/sign', (req, res) =>{
      ...
      console.log("Request was valid, by: " + fetched.name);
      var returnObject = {
  ...
  app.post("/update", (req, res) => {
    ...
    const md5Hash = md5File.sync('binary/release.exe');
    console.log(md5Hash);
  ...
  app.get("/bin/:game", (req, res)=>{
    ...
    let gameObject;
    extra.gameObjects.forEach(_gameObject => {
      if (game == _gameObject.gameName){
        console.log("Got a hit with " + _gameObject.processName);
    ...
    if (gameObject.processName == "csgo"){
      let file = fs.readFileSync('binary/injector.exe');
  ...
  app.get("/update/:id", (req, res)=>{
    ...
    let file = fs.readFileSync('binary/release.exe');
    res.write(file, "binary");
    res.end(null, 'binary');
